...
const getGoodsData = async () => {
  const { result } = await getGoodsAPI(query.id)
  ...
  localdata.value = {
    ...
    sku_list: result.skus.map((sku) => ({
      ...
      image: sku.picture,
      price: sku.price * 100,
      sku_name_arr: sku.specs.map((skuSpec) => skuSpec.valueName),
      stock: sku.inventory,
    })),
    spec_list: result.specs.map((spec) => ({
      name: spec.name,
...
const onChange: UniHelper.SwiperOnChange = (e) => {
  currentMainIndex.value = e.detail.current
}
// 轮播图片点击
    ...
    <view class="goods">
      ...
      <view class="preview">
        ...
        <view class="indicator">
          <text class="current">{{ currentMainIndex + 1 }}</text>
      ...
      <!-- 操作面板 -->
      <view class="action">
        <view class="item arrow" @tap="isShowSku = !isShowSku">
