...
const selectedAddress = computed(() => {
  return (
    addressStore.selectedAddress || orderResult.value?.userAddresses.find((item) => item.isDefault)
  )
})
...
const onOrderSubmit = async () => {
  ...
  const {
    ...
    goods: orderResult.value!.goods.map((item) => ({ count: item.count, skuId: item.skuId })),
    payChannel: 2,
    payType: 1,
  })

  // 关闭当前页面，跳转到订单详情页面 并传递订单id
...
<template>
  <scroll-view scroll-y class="viewport">
    ...
    <view class="goods">
      <navigator
        v-for="item in orderResult?.goods"
        :key="item.skuId"
    ...
    <!-- 支付金额 -->
    <view class="settlement" :style="{ marginBottom: '100rpx' }">
      <view class="item">
